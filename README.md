...
For demo desync work against the original Linux DOOM source tree in `../doom-source`, `scripts/demo_trace_compare.sh` runs the reference runtime with `-tracedemo`, runs GD-DOOM with `-trace-demo-state`, and compares the resulting tic traces with `cmd/demotracecmp`.
