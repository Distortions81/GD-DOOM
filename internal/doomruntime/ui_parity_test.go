...
func TestMapThingSpriteName_HidesLostSoulAfterDeathAnimation(t *testing.T) {
	g := &game{
		opts: Options{
			SpritePatchBank: map[string]WallTexture{
				"SKULF0": {Width: 1, Height: 1, RGBA: []byte{255, 255, 255, 255}},
				"SKULK0": {Width: 1, Height: 1, RGBA: []byte{255, 255, 255, 255}},
			},
		},
		thingDead:      []bool{true, true},
		thingDeathTics: []int{1, 0},
	}
	th := mapdata.Thing{Type: 3006, X: 0, Y: 0, Angle: 0}
	if got := g.mapThingSpriteName(0, th); got == "" {
		t.Fatal("lost soul should remain visible while death animation is active")
	}
	if got := g.mapThingSpriteName(1, th); got != "" {
		t.Fatalf("lost soul corpse sprite=%q want hidden after death animation", got)
	}
}
